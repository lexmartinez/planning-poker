    ...
    init: (win) => {
        ...
        ipcMain.on('session-auth', (event, arg) => {
            ...
            Session.findOne({sid}, (error, session) => {
              if (error) { 
                ...
                if (session && (session.team.indexOf(email) !== -1 || session.host === email)) {
                  const {_id, sid, host, team, backlog, status, current, voting} = session
                  event.sender.send('session-auth-reply', {response: {id:_id, sid, host, team, backlog, status, current, voting}});
                } else {
                  event.sender.send('session-auth-reply', {response: false});
                }
              }
            ...
            Session.findOne({sid}, (error, session) => {
              if (error) { 
                ...
                event.sender.send('get-session-reply', {error});
                  ...
                  const {_id, sid, host, team, backlog, status, current, voting} = session
                  event.sender.send('get-session-reply', {session: {id:_id, sid, host, team, backlog, status, current, voting}});
              }
            });
          });
          ...
          ipcMain.on('create-session', (event, host) => {
            ...
            const team = [host]
            const session = new Session({sid, host, team, backlog: [], voting: [], status: 'CREATED', current: 0})
            session.save().then((response) => {
              const {_id, sid, host, team, backlog, status, current, voting} = response
              event.sender.send('create-session-reply', {id:_id, sid, host, team, backlog, status, current, voting});
            });
          });
          ...
          ipcMain.on('handle-vote', (event, {user, session, vote}) => {
            const sid = session.sid
            const voting = session.voting
            const current = voting[session.current]
            if (!current) {
